...
- **Framework**: **Next.js 16 (App Router)** - 利用 React Server Components (RSC) 和 Server Actions。
...
- **Testing**: **Vitest + Playwright** - 单元、集成与浏览器端到端测试。

最后非常感谢陪伴我的ChatGPT Codex, Claude 3.5 3.7 4以及Gemini 3！

## 发布与数据库升级

部署前请先运行完整质量门禁，并用只读检查确认目标数据库与代码版本兼容：
...
数据库升级、验证和回退注意事项见 [部署运行手册](docs/DEPLOYMENT.md)。
