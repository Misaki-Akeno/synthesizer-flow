...
export class ModuleFactory implements IModuleFactory {
   ...
   */
  private async getModuleClass(typeId: string, config: ModuleConfiguration): Promise<ModuleConstructor> {
    // 首先检查已注册的模块类
    ...
    if (config.metadata.moduleConstructor) {
      const constructor = config.metadata.moduleConstructor as ModuleConstructor;
    ...
    if (config.metadata.moduleClass) {
      try {
        ...
        // 尝试获取导出的模块类
        const ExportedClass = moduleExport.default || moduleExport[typeId + 'Module'];
        ...
        throw new Error(`无法导入模块类 ${typeId}: ${(error as Error).message}`);
      }
    }
