...
import { ModuleBase, ModuleCategory, ModuleMetadata, ModuleConfiguration, DataType } from '@/types/module';
...
class MockModule implements ModuleBase {
  ...
  constructor(params: any) {
    ...
    // 在构造函数中创建可跟踪的模拟函数
    this.initialize = vi.fn().mockImplementation(() => MockModule.initializeMock());
...
describe('ModuleFactory', () => {
  ...
  describe('create', () => {
    ...
    it('使用提供的 instanceId 创建模块', async () => {
      ...
      const moduleInstance = await moduleFactory.create('test-module', 'custom-id');
    ...
    it('当模块类型未注册时抛出错误', async () => {
      ...
      // 创建模块实例应该抛出错误
      await expect(moduleFactory.create('unknown-module'))
        .rejects.toThrow('模块类型 unknown-module 未注册');
    ...
    it('当模块初始化失败时应该处理异常', async () => {
      ...
      // 创建模块实例应该失败
      await expect(moduleFactory.create('test-module'))
  ...
  describe('destroy', () => {
    ...
    it('当模块不存在时抛出错误', async () => {
      // 销毁不存在的模块应该抛出错误
      await expect(moduleFactory.destroy('non-existent-id'))
    ...
    it('当模块销毁失败时处理异常', async () => {
      ...
      // 销毁模块实例应该失败
      await expect(moduleFactory.destroy('test-id-123'))
        .rejects.toThrow('销毁失败');
