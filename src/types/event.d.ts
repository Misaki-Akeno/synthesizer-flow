...
// 模块服务事件
export interface ModuleServiceInitializedEvent {
  time: string; // ISO 时间字符串
}

// 模块创建请求事件
export interface ModuleCreateRequestEvent {
  typeId: string;
  instanceId?: string;
  position?: Position;
}
...
// 模块创建失败事件
export interface ModuleCreateFailedEvent {
  typeId: string;
  instanceId?: string;
  error: unknown;
}
...
// 模块销毁请求事件
export interface ModuleDestroyRequestEvent {
  moduleId: string;
}
...
// 模块销毁失败事件
export interface ModuleDestroyFailedEvent {
  moduleId: string;
  error: unknown;
}

// 模块销毁成功事件
export interface ModuleDisposedEvent {
  moduleId: string;
}
...
// 模块初始化失败事件
export interface ModuleInitializeFailedEvent {
  moduleId: string;
  error: unknown;
}
...
// 触发同步事件
export interface TriggerSyncEvent {
  targetId: string;
  value?: number; // 添加触发值
...
// 连接断开事件
export interface ConnectionBrokenEvent {
  connectionId: string;
  sourceId: string;
  targetId: string;
  sourceHandle?: string;
