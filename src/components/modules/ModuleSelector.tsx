...
export const ModuleSelector: React.FC<ModuleSelectorProps> = ({
  ...
  const handleModuleDragStart = (
    ...
    if (
      e.target &&
      (e.target as HTMLElement).closest('.module-selector-handle')
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      if (isDraggingModule && draggedModule && isOutsideContainer) {
        ...
        onModuleSelect(draggedModule.moduleType, draggedModule.moduleLabel, {
          x: canvasX,
          y: canvasY,
        });
      }
                  ...
                  {filteredModules.map((module) => (
                    <div
                      ...
                      className={cn(
                        ...
                        isDraggingModule &&
                          draggedModule?.moduleType === module.type &&
                          'opacity-50'
                      )}
                      onMouseDown={(e) =>
                        handleModuleDragStart(e, module.type, module.label)
                      }
                      onDoubleClick={() =>
                        handleDoubleClick(module.type, module.label)
                      }
                    >
      ...
      {isDraggingModule && draggedModule && (
        <div
          className={cn(
            ...
            isOutsideContainer
              ? 'bg-white/90 dark:bg-gray-800/90 border-blue-400'
              : 'bg-gray-100/90 dark:bg-gray-700/90 border-gray-300'
          )}
          style={{
