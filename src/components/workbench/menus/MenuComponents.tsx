...
export const MenuSection: React.FC<MenuSectionProps> = ({ id, title, label }) => (
	<ContextMenu.Label
		key={id}
...
export const MenuItem: React.FC<MenuItemProps> = ({ id, label, onClick, onClose }) => (
	<ContextMenu.Item
		key={id}
		className={cn(
			...
			<ContextMenu.Portal>
				...
				>
					{items.map((subItem, index) => (
						<React.Fragment key={index}>{renderMenuItem(subItem)}</React.Fragment>
					))}
				</ContextMenu.SubContent>
			</ContextMenu.Portal>
		</ContextMenu.Sub>
