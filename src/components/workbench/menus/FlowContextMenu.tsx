...
import { MenuSection, MenuItem, SubMenu } from '@/components/workbench/menus/MenuComponents';
...
const generateMenuFromModules = (onAddNode: AddNodeCallback): MenuConfigItem[] => {
	...
	const categories = Array.from(
		new Set(allModules.map(m => (m.metadata?.category as string) || '').filter(Boolean))
	);
	...
	categories.forEach(category => {
		...
		const moduleItems: MenuConfigItem[] = modulesInCategory.map(module => {
			...
			if (moduleMetadata && 'presets' in moduleMetadata && Array.isArray(moduleMetadata.presets)) {
...
const FlowContextMenu: React.FC<FlowContextMenuProps> = ({ top, left, onAddNode, onClose }) => {
	...
	const renderMenuItem = React.useCallback((item: MenuConfigItem): React.ReactNode => {
		const { type, id } = item;
		if (!item) return null;
		...
		switch (type) {
			...
			case MENU_ITEM_TYPES.ITEM:
				return <MenuItem key={id} {...(item as MenuSimpleItem)} onClose={onClose} />;
	...
	return (
		<div className="absolute z-50" style={{ top, left }} onClick={(e) => e.stopPropagation()}>
			<ContextMenu.Root modal={false} onOpenChange={(open) => !open && onClose()}>
				<div
					style={{
						width: '1px',
						height: '1px',
