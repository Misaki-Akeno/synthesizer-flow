...
function getNoteLaneLabel(lane: LaneKind): string {
  if (lane === 'velocity') return 'Velocity';
  if (lane === 'pressure') return 'Pressure';
  if (lane === 'timbre') return 'Timbre';
  return 'Pitch';
}

function applyLaneValue(note: MidiNote, lane: LaneKind, normalizedValue: number): MidiNote {
...
export function MidiClipEditorPanel() {
  ...
  const node = useFlowStore((state) => state.nodes.find((item) => item.id === moduleId));
  ...
  const [snapTicks, setSnapTicks] = useState(() => Math.round(initialClip.ppq / 4));
  const activeSnapTicks = snapOptions.some((option) => option.value === snapTicks)
    ? snapTicks
    : (snapOptions[2]?.value ?? Math.round(clip.ppq / 4));
  const selectedNote = clip.notes.find((note) => note.id === selectedNoteId) ?? null;
  const bpmValue = node?.data.module?.getParameterValue('bpm');
  const runningValue = node?.data.module?.getParameterValue('running');
  const bpm = typeof bpmValue === 'number' && Number.isFinite(bpmValue) ? bpmValue : 120;
  ...
  const saveClip = useCallback(
    ...
    [moduleId, updateModuleParameter]
  );

  const updateNote = useCallback(
    (noteId: string, updater: (note: MidiNote) => MidiNote) => {
      saveClip({
        ...clip,
        notes: clip.notes.map((note) => (note.id === noteId ? updater(note) : note)),
      });
    },
    [clip, saveClip]
  );

  const tickToX = useCallback((tick: number) => (tick / lengthTicks) * rollWidth, [lengthTicks, rollWidth]);
  const xToTick = useCallback((x: number) => (x / rollWidth) * lengthTicks, [lengthTicks, rollWidth]);
  const pitchToY = useCallback((midi: number) => (MAX_MIDI - midi) * ROW_HEIGHT, []);
  const yToPitch = useCallback((y: number) => clamp(MAX_MIDI - Math.floor(y / ROW_HEIGHT), MIN_MIDI, MAX_MIDI), []);
  ...
  const commitNoteFromPoint = useCallback(
    (event: ReactPointerEvent) => {
      ...
      const startTick = clamp(quantizeTick(xToTick(point.x), activeSnapTicks), 0, Math.max(0, lengthTicks - activeSnapTicks));
    ...
    [activeSnapTicks, clip, getPointerPosition, lengthTicks, moduleId, saveClip, xToTick, yToPitch]
  );
  ...
  const duplicateSelectedNote = useCallback(() => {
    ...
    const nextStartTick = clamp(selectedNote.startTick + activeSnapTicks, 0, Math.max(0, lengthTicks - selectedNote.durationTicks));
    const copy: MidiNote = {
      ...selectedNote,
  ...
  const quantizeSelection = useCallback(() => {
    ...
    saveClip({
      ...
      notes: clip.notes.map((note) => {
        ...
        const startTick = clamp(quantizeTick(note.startTick, activeSnapTicks), 0, Math.max(0, lengthTicks - 1));
        const durationTicks = Math.max(activeSnapTicks, quantizeTick(note.durationTicks, activeSnapTicks));
        return {
          ...note,
          startTick,
  ...
  const handleNotePointerDown = (
    ...
    if (tool === 'erase') {
      saveClip({ ...clip, notes: clip.notes.filter((item) => item.id !== note.id) });
      setSelectedNoteId(null);
      return;
    }
  ...
  const updateLaneFromPointer = useCallback(
    (event: PointerEvent | ReactPointerEvent, noteId: string) => {
      ...
      if (!rect) return;
      const normalized = clamp(1 - (event.clientY - rect.top) / rect.height, 0, 1);
      updateNote(noteId, (note) => applyLaneValue(note, lane, normalized));
    },
    [lane, updateNote]
  );

  const handleLanePointerDown = (event: ReactPointerEvent<HTMLDivElement>, note: MidiNote) => {
    if (event.button !== 0) return;
    event.stopPropagation();
    setSelectedNoteId(note.id);
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const deltaRows = Math.round((event.clientY - dragState.startClientY) / ROW_HEIGHT);
      ...
      updateNote(dragState.noteId, () => {
        if (dragState.kind === 'resize-start') {
          const maxStartTick = original.startTick + original.durationTicks - minDuration;
          const startTick = clamp(original.startTick + deltaTicks, 0, maxStartTick);
          return {
            ...original,
            startTick,
            durationTicks: original.startTick + original.durationTicks - startTick,
          };
        }

        if (dragState.kind === 'resize-end') {
          const durationTicks = clamp(original.durationTicks + deltaTicks, minDuration, lengthTicks - original.startTick);
        ...
        return {
          ...
          startTick: clamp(original.startTick + deltaTicks, 0, Math.max(0, lengthTicks - original.durationTicks)),
        };
      });
    };
  ...
  }, [activeSnapTicks, dragState, lengthTicks, rollWidth, updateLaneFromPointer, updateNote]);
  ...
  if (!moduleId || !node?.data.module) {
    return (
      <div className="flex h-full items-center justify-center bg-background text-sm text-muted-foreground">
        Select a MIDI Clip module to edit.
      </div>
    );
  }

  const noteRows = Array.from({ length: MAX_MIDI - MIN_MIDI + 1 }, (_, index) => MAX_MIDI - index);
    ...
    <div className="grid h-full grid-rows-[46px_1fr_84px] bg-background text-foreground">
      <div className="flex min-w-0 items-center gap-2 border-b bg-card px-3">
        ...
        <div className="mr-1 min-w-0">
          <div className="truncate text-sm font-semibold leading-5 text-foreground">{node.data.label}</div>
          <div className="text-[11px] leading-3 text-muted-foreground">
            {clip.notes.length} notes · {clip.bars} bars · {clip.timeSignature[0]}/{clip.timeSignature[1]}
          </div>
        </div>
        ...
        <ToolButton active={isRunning} label={isRunning ? 'Stop' : 'Play'} onClick={() => updateModuleParameter(moduleId, 'running', !isRunning)}>
        ...
        <Select value={String(activeSnapTicks)} onValueChange={(value) => setSnapTicks(Number(value))}>
        ...
        <ToolButton label="Quantize" onClick={quantizeSelection}>
          <Scissors className="h-4 w-4" />
        </ToolButton>
        <ToolButton label="Duplicate" disabled={!selectedNote} onClick={duplicateSelectedNote}>
          <Copy className="h-4 w-4" />
        </ToolButton>
        <ToolButton label="Delete" disabled={!selectedNote} onClick={removeSelectedNote}>
          <Trash2 className="h-4 w-4" />
        </ToolButton>
      </div>

      <div className="grid min-h-0 grid-cols-[1fr_var(--inspector-width)]" style={{ '--inspector-width': `${INSPECTOR_WIDTH}px` } as React.CSSProperties}>
        <div ref={rollScrollRef} className="min-w-0 overflow-auto bg-muted/20">
          <div
            ref={rollRef}
            className="relative"
            style={{ width: KEYBOARD_WIDTH + rollWidth, height: TIMELINE_HEIGHT + gridHeight }}
          >
            <div
              className="sticky top-0 z-30 border-b bg-card"
              style={{ left: 0, width: KEYBOARD_WIDTH + rollWidth, height: TIMELINE_HEIGHT }}
            >
              <div className="sticky left-0 z-40 flex h-full items-center border-r bg-card px-2 text-[10px] text-muted-foreground" style={{ width: KEYBOARD_WIDTH }}>
                Key
              </div>
              {Array.from({ length: beatCount + 1 }, (_, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    index % clip.timeSignature[0] === 0 ? 'border-chart-5/45 text-chart-5' : 'border-border text-muted-foreground'
                  )}
            ...
            <div className="absolute left-0 z-20" style={{ top: TIMELINE_HEIGHT, width: KEYBOARD_WIDTH, height: gridHeight }}>
              {noteRows.map((midi) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isBlackKey ? 'bg-muted text-foreground' : 'bg-card text-muted-foreground'
                    )}
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      onPointerDown={(event) => handleNotePointerDown(event, note, 'resize-start')}
                    />
                    <span className="min-w-0 truncate">{midiToNoteName(note.midi)}</span>
                    <span
                      className="absolute right-0 top-0 h-full w-2 cursor-ew-resize rounded-r-[5px] bg-black/10 opacity-0 group-hover:opacity-100"
                      onPointerDown={(event) => handleNotePointerDown(event, note, 'resize-end')}
                    />
                  </div>
                );
              })}
        ...
        <Inspector
          ...
          note={selectedNote}
          lane={lane}
          onChangeNote={(updater) => {
            if (!selectedNote) return;
            updateNote(selectedNote.id, updater);
          }}
        />
      </div>

      <div className="grid min-h-0 grid-cols-[1fr_var(--inspector-width)] border-t bg-card" style={{ '--inspector-width': `${INSPECTOR_WIDTH}px` } as React.CSSProperties}>
        <div className="min-w-0 overflow-x-auto overflow-y-hidden px-3 py-2">
          <div className="mb-2 flex items-center gap-2">
            <div className="w-[70px] text-[10px] font-medium uppercase text-muted-foreground">{getNoteLaneLabel(lane)}</div>
            <Select value={lane} onValueChange={(value) => setLane(value as LaneKind)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="timbre">Timbre</SelectItem>
                <SelectItem value="pitchBend">Pitch Bend</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div ref={laneRef} className="relative" style={{ width: KEYBOARD_WIDTH + rollWidth, height: 42 }}>
            <div className="sticky left-0 z-10 h-full border-r bg-card" style={{ width: KEYBOARD_WIDTH }} />
            <div className="absolute rounded-sm bg-muted/45" style={{ left: KEYBOARD_WIDTH, width: rollWidth, height: 42 }}>
              {lane === 'pitchBend' && <div className="absolute left-0 right-0 top-1/2 h-px bg-chart-2/45" />}
              {clip.notes.map((note) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      selected ? 'border-chart-2/70 bg-chart-2' : 'border-chart-5/70 bg-chart-5/85'
                    )}
                    style={{
                      left: tickToX(note.startTick),
                    ...
                    title={`${getNoteLaneLabel(lane)} ${value.toFixed(2)}`}
                    onPointerDown={(event) => handleLanePointerDown(event, note)}
                  />
                );
              })}
            </div>
        ...
        <div className="border-l bg-card px-3 py-2 text-[11px] text-muted-foreground">
          <div className="mb-1 text-xs font-semibold text-foreground">MPE lanes</div>
          <div>Velocity, pressure, timbre, pitch bend</div>
        </div>
      </div>
    </div>
  );
...
function Inspector({
  ...
  note: MidiNote | null;
  lane: LaneKind;
  onChangeNote: (updater: (note: MidiNote) => MidiNote) => void;
}) {
  if (!note) {
    return (
      <div className="border-l bg-card p-3">
        <div className="mb-3 text-xs font-semibold text-foreground">Inspector</div>
        <div className="text-xs text-muted-foreground">No note selected</div>
      </div>
    );
  }
    ...
    <div className="min-h-0 overflow-y-auto border-l bg-card p-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <InspectorNumber
          ...
          min={0}
          max={127}
          onChange={(value) => onChangeNote((current) => ({ ...current, midi: Math.round(value) }))}
        />
        <InspectorNumber
          label="Start"
          ...
          min={0}
          max={lengthTicks - 1}
          onChange={(value) => onChangeNote((current) => ({ ...current, startTick: Math.round(value) }))}
        />
        <InspectorNumber
          label="Duration"
          ...
          onChange={(value) => onChangeNote((current) => ({ ...current, durationTicks: Math.round(value) }))}
        />
      ...
      <div className="mt-4 space-y-4">
        <InspectorSlider
          ...
          max={1}
          step={0.01}
          onChange={(value) => onChangeNote((current) => ({ ...current, velocity: value }))}
        />
        <InspectorSlider
          label="Pressure"
          ...
          max={1}
          step={0.01}
          onChange={(value) => onChangeNote((current) => ({ ...current, pressure: value }))}
        />
        <InspectorSlider
          label="Timbre"
          value={note.timbre ?? 0}
          ...
          max={1}
          step={0.01}
          onChange={(value) => onChangeNote((current) => ({ ...current, timbre: value }))}
        />
        <InspectorSlider
          label="Pitch bend"
          ...
          max={1}
          step={0.01}
          onChange={(value) => onChangeNote((current) => ({ ...current, pitchBend: value }))}
        />
      </div>

      <div className="mt-4 rounded-md border bg-muted/40 p-2 text-[11px] text-muted-foreground">
        Active lane: {getNoteLaneLabel(lane)}
      </div>
    </div>
  );
}
