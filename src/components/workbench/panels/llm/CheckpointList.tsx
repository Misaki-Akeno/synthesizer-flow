...
export function CheckpointList({ onRestore }: CheckpointListProps) {
  ...
  const handleSaveTitle = async (id: string) => {
    ...
    const res = await updateCheckpointTitle(id, editTitle);
    if (res.success) {
      toast.success('标题已更新');
      setEditingId(null);
      loadCheckpoints();
    ...
    <div className="space-y-4">
      ...
      <ScrollArea className="h-[300px] pr-4">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">暂无存档</div>
        ) : (
