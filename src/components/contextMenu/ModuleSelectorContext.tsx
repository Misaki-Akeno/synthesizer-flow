...
const ModuleSelectorContext = createContext<ModuleSelectorContextType | null>(null);

export const useModuleSelectorContext = () => {
  const context = useContext(ModuleSelectorContext);
  if (!context) {
    throw new Error('useModuleSelectorContext must be used within a ModuleSelectorProvider');
  }
  return context;
};
...
export const ModuleSelectorProvider: React.FC<ModuleSelectorProviderProps> = ({ children }) => {
  ...
  // 处理模块选择
  const handleModuleSelect = (type: string, label: string, position: { x: number; y: number }) => {
  ...
  return (
    <ModuleSelectorContext.Provider value={{ showModuleSelector, hideModuleSelector }}>
      {children}
